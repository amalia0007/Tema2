package com.tema2.tema2;

import com.tema2.tema2.model.Person;
import com.tema2.tema2.model.Room;
import com.tema2.tema2.model.Track;
import com.tema2.tema2.repository.PersonRepository;
import com.tema2.tema2.repository.RoomRepository;
import com.tema2.tema2.repository.TrackRepository;
...
public class MainController {
    ...
    /**
     * Continuati exemplul de la laborator si implementati entitatea Room si legaturile cu
     * Track pentru a putea gestiona alocarea trackurilor pe sali.
     * Implementati operatiile de create, read, update, delete pentru resursa de tip Room
     * si afisati ce trackuri sunt planificate pentru fiecare sala.
     **/
    ...
    public ResponseEntity<Void> addRooms(String roomName, Integer chairs) {
        Room room = new Room(roomName, chairs);

        roomRepository.save(room);

        URI uri = WebMvcLinkBuilder.linkTo(MainController.class).slash("rooms").slash(room.getRoomId()).toUri();
        return ResponseEntity.created(uri).build();
    }

    @PutMapping(path = "/rooms/{roomId}")
    public ResponseEntity<Void> changeRoom(@PathVariable("roomId") long id, @RequestBody Room room) {
    ...
    public ResponseEntity<Void> closeRoom(@PathVariable("roomId") long id) {
        Optional<Room> existingRoom = roomRepository.findById(id);
        if (existingRoom.isPresent()) {
